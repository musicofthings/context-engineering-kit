#!/usr/bin/env bash
# scripts/session_finalize.sh
# End-of-session work: force a final handover, sync worktree state up to the
# main checkout, append to history, and commit session files to git so they are
# available on other devices/subscriptions.
#
# This used to be .claude/hooks/session-end.sh, running inline on SessionEnd.
# It cannot run there any more: SessionEnd hooks share a 1.5-second budget, and
# per the hooks reference "Timeouts set on plugin-provided hooks don't raise the
# budget" — so the manifest's `timeout: 30` was ignored for every plugin install
# and this script was killed partway through. Codex caps SessionEnd even harder
# (1s default, 3s maximum, always synchronous).
#
# session-end.sh now detaches this script so it outlives the hook's budget.
# Run it inline instead by exporting CEK_SESSION_END_SYNC=1.
...
if declare -f cek_auto_save_init >/dev/null 2>&1; then
  ...
  if [ "$NEED_SAVE" = true ] && declare -f cek_execute_save_pipeline >/dev/null 2>&1; then
    cek_execute_save_pipeline "session-end" "unknown" || log "auto-save at session finalize failed (non-fatal)"
  fi
fi
